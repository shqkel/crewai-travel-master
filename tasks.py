...
"""
Task는 프롬프트 엔지니어링적으로 지속적으로 개선해 나가야 한다.
"""
...
initial_travel_plan_task = Task(
    ...
    expected_output="""
    최종 여행 일정 계획에 필요한 모든 정보 수집 완료해야 합니다.
...
final_coordinator_task = Task(
    ...
    expected_output="""
한국어로 작성된 고객에게 전달할 최종 여행 일정 계획서(항공편 상세, 숙소 상세, 전체 비용, 일자별 일정표, 상세 예산표, 추가 정보)

## 예산범위 검증
- 계획한 [총 예산]이 사용자 요청 [총 예산] 내에 있으면 성공입니다.
- 계획한 [총 예산]이 사용자 요청 [총 예산] 밖이면 실패이니, 여행계획을 다시 작성합니다.
