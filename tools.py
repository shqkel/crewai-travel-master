...
class NearbyPlacesTool(BaseTool):
    ...
    def find_nearby_places(self, location, radius):
        ...
        params = {
            "location": f"{lat},{lng}",
            "radius": radius,
            "type": "tourist_attraction", # 장소유형 검색 https://developers.google.com/maps/documentation/places/web-service/place-types?hl=ko&_gl=1*zxa2ql*_up*MQ..*_ga*MTkzODgzODIyMy4xNzQ1MTI2NzI0*_ga_NRWSTWS78N*MTc0NTEyNjcyNC4xLjEuMTc0NTEyNjk1NS4wLjAuMA..
            "language": "ko",
            "key": os.getenv("GOOGLE_API_KEY")
        }
        response = requests.get(url, params=params).json()
